...
os.chdir('D:/lastBox/관련기사 수집/관련기사수집 xlsx')
...
def crawler(company, company_code, maxpage):
    ...
    while page <= int(maxpage): 
        ...
        print("다운 받고 있습니다------")
        df_result.to_csv(f'{company}_page_' + str(page) + f'_{date_time}.csv', mode='w', encoding='utf-8-sig') 
...
def convert_to_code(company, maxpage):
    ...
    data = pd.read_csv('D:/lastBox/종목코드/company_list.txt', dtype=str, sep='\t')   # 종목코드 추출 
    ...
    if bool(re.match(pattern, company)) == True:         # Input에 이름으로 넣었을 때  
        ...
        crawler(company, company_code, maxpage)
